...
Regions resembling retroviral gag, pol and env genes are identified based on their level of similarity to a database of known complete or partial retroviral ORFs.

A library of 384 representative amino acid sequences is provided, however the user can also provide a custom database.
...
No compliation is required, just add the "src" directory to your path or use the full path to src/pipeline_ERVsearch. If installed using pip you should be able to call `ERVsearch` directly.
...
The main input file is sequence file in `fasta` format (https://en.wikipedia.org/wiki/FASTA_format) containing DNA sequences from the genome of interest which you wish to screen for ERV-like regions. This would usually be a reference or de novo assembled genome.

Reference genome sequences are available from http://www.ensembl.org/info/about/species.html and https://genome-euro.ucsc.edu/index.html (amongst others). De novo assemblies can also be used, as can any set of DNA sequences in which you wish to identify ERVs.

To be used as an input file, the reference genome needs to be combined into a single FASTA file.
For Ensembl genomes, this would usually be the` GENOMEID.dna.toplevel.fa.gz `file from the "download DNA sequence" page for the appropriate organism, substituting `GENOMEID` for the genome ID.
For UCSC genomes this would be `GENOMEID.fa.gz` from the bigZips directory for this organism on the FTP server, substituting `GENOMEID` for the genome ID.
...
A default `pipeline.ini` file is available in src/pipeline.ini.
...
|------------------|--------------------|---------------|
|`input`|-|-|
|`genome`|path to genome to screen|`/home/katy/ERVsearch_screen/mygenome.fasta`|
|`database`|-|-|
|`path_to_ERVsearch`| path to the main ERVsearch folder|`/home/katy/ERVsearch`|
|`paths`|-|-|
|`path_to_exonerate`|path to the exonerate executable|`/home/katy/Exonerate2.4.0/bin/exonerate
|`path_to_usearch`|path to the USEARCH executable| `/home/katy/USEARCH/usearch`|
...
A list of chromosome names to include. The names should the names in the fasta file cropped at the first space,  e.g. NW_006711271.1 Panthera tigris altaica isolate TaeGuk unplaced genomic scaffold should just be listed as NW_006711271.1. The names are case sensitive and need to be identical to those in the fasta file.
